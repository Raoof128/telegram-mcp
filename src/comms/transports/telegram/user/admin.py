...
except ``message.send`` (D14), whose ``random_id`` is derived from it (A20) and which runs
through ``user.send`` with its one reconciliation.
"""
...
def _sent(delivered: DeliveryResult) -> ProviderResult:
    """A keyed send's delivery result as an operation outcome; the ref is the message id."""
    if delivered.kind is ResultKind.ACCEPTED:
        ref = delivered.provider_message_ref
        message_id = ref.rsplit(":", 1)[1] if ref else None
        return ProviderResult("SUCCEEDED", None, provider_ref=message_id)
    if delivered.kind is ResultKind.FAILED_PERMANENT:
        return ProviderResult("FAILED", "NOT_AUTHORIZED")
    if delivered.kind is ResultKind.FAILED_TRANSIENT:
        detail = {"retry_after": delivered.retry_after} if delivered.retry_after else {}
        return ProviderResult("FAILED", "RATE_LIMITED", detail=detail)
