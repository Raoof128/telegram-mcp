...
from comms.core.campaigns.drafts import LifecycleError, NotFound, load
...
def cancel_sending_in_tx(conn: Any, cmp: str, *, now: datetime) -> CancelReport:
    ...
    lifecycle = conn.execute("SELECT lifecycle FROM campaigns WHERE ref = ?", (cmp,)).fetchone()[0]
    append_event(
        conn,
        "campaign.cancelled",
        cmp,
...
def retry_failed_in_tx(conn: Any, cmp: str, *, now: datetime, cap: int = 5) -> RetryReport:
    ...
    requeued = sum(
        reduce(conn, job_id, None, Evidence.RETRY, None, now=now, cap=cap).disposition == "applied"
...
def retry_failed(conn: Any, cmp: str, *, now: datetime, cap: int = 5) -> RetryReport:
    """Return provably-unsent (FAILED_TRANSIENT) jobs below the cap to PENDING (§7.2)."""
    with write_tx(conn):
        return retry_failed_in_tx(conn, cmp, now=now, cap=cap)


def resolve_outcome_in_tx(
    ...
    try:
        ...
        raise NotFound("unknown job") from None
    row = conn.execute(
...
def resolve_outcome(
    ...
    """An operator's statement about an OUTCOME_UNKNOWN job (§7.2)."""
    with write_tx(conn):
        resolve_outcome_in_tx(conn, job_ref, verdict, now=now)
