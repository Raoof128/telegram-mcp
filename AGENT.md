...
- **Follow-ups:** Run `pairing generate` and one real Touch ID approval (both platform-gated), then re-point Plan 2a's Phase-2J join-gate harness at this driver and drive the remaining seven scenarios. No pairing record, no Enclave key, no loaded LaunchAgent, no Telegram access, no production claim.
