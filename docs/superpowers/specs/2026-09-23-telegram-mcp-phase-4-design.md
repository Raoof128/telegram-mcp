...
Every item below came from running the 4c code. The items marked "from the plan gauntlet" or "from the external review" were found by reading the plan against the spec and the pinned Telethon source. Each was then confirmed by a test that failed before the fix.
...
- **Owner chat-kind switches are enforced at retrieval for search.** They need dialog data that step 2 cannot fetch. In each call, one batched `GetPeerDialogs` checks every active peer before it is searched, including peers a continuation resumes mid-walk. An excluded peer is never searched. A peer the entity cache cannot reach is never searched either, and it makes the result `telegram_partial`, never `complete`.
...
- **The cursor state has one value rule per key, as a table.** A `per_peer` entry holds only `offset_id`. A `per_peer` ref that no longer resolves refuses; it never reads as exhausted.
- **A page ends on Telegram's own signal, never on a short page (from the plan gauntlet; refines §4.2).** Telethon 1.45.0 (`client/messages.py:213-225`) documents that channels withhold messages, so fewer results than requested does not mean the end. One adapter rule, `_page_end`, serves history and search alike. A page is the last only if it is empty, or a non-slice `messages.Messages`, or its highest id is within the limit. The next offset comes only from the chat's own entries. A foreign entry marks the page inexact. This also corrects 4b's `fetch_history`, which dropped `get_messages`' cursor on a short page.
- **Coverage is honest across a whole continuation (from the plan gauntlet; refines §4.4).** Spec §23D permits `complete` only if *every* eligible peer reached Telegram's end. So uncertainty is sticky: an `inexact` page or an unreachable peer sets the cursor flag `uncertain`, and every later page reports `telegram_partial`. `peers_scanned` counts peers searched to their end, never merely started (the window starts 64 before any request).
- **Coverage counters are measured, and owner scope shapes eligibility (from the external review; refines §4.4).** `telegram_rpcs` is the call's `WorkBudget.used`, so dialog checks count. `hits_examined` is the raw entries Telegram returned, before any filtering. A peer that the owner's live scope excludes leaves `eligible_peers`, globally and per project, as spec §21.4 defines the universe after owner scope. A peer the entity cache cannot reach stays eligible and unscanned. Both counts are cumulative in the cursor (`scanned_counts`, `excluded_counts`). The universe digest still binds the authority-derived candidate universe. Admission is decided live when a peer is reached and re-checked on every call.
- **At most 10 search pages per call (from the external review).** Spec §13.2 sets it separately from the 20 RPCs and 500 hits. The closed `partial_reasons` list has no page reason, so the bound reports `rpc_budget`, the work-bound reason.
- **Both response caps, one rule (from the external review).** §13.2's 32,000-codepoint combined-text cap was enforced nowhere, including 4b's `get_messages`. `bounds.PageBudget` admits a record only within both the byte cap and the codepoint cap, counting every string in `data`. `fit()` and the search's `accept()` use it before the disclosure commit.
- **Forum classification uses the dialog (from the external review).** `get_context` classifies a chat as a forum if the fetched dialog says so or the by-id response does. Erring toward a forum only narrows the window to one topic.
- **The epoch edge (from the external review).** A widened `since` at or before the epoch sends no lower bound, and the exact post-filter decides. Over-fetch is safe; under-fetch is not.
- **One RFC 3339 parser (from the plan gauntlet).** `validation.parse_time` accepts the lowercase `t`/`z` that RFC 3339 §5.6 allows and jsonschema accepts. `until` is exclusive and Telegram dates are whole seconds, so a fractional `until` rounds up to the next second.
- **Smaller decisions:**
