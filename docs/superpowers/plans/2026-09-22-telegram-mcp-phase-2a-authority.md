...
- [ ] **Step 7: Implement `bootstrap.py`.** `start --local/--chatgpt/--all` (spawns runtime as configured user via the installed service definition; refuses double start), `stop` (admin-socket shutdown with timeout → SIGTERM → reap; no-op success while OFF), `status` (OFF/STARTING/READY/STOPPING + mode + pid, never secrets). Child PIDs (tunnel-client, consent UI) tracked in a runtime-owned PID file; at start a live PID with exact cmdline-plus-UID match is terminated and started fresh.
...
- [ ] **Step 2: Run; expect missing-module failure.** Implement `mint_opaque_ref`: 130 CSPRNG bits → 26 lowercase unpadded Base32 chars. Reuse in `consent/challenge.py` (replace the Task-3 local minter; delete duplication, keep tests green).
