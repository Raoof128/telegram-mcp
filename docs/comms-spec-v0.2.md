...
| §27.1 error codes | `CONSENT_DENIED` and `CONSENT_UNAVAILABLE` are frozen codes in the error enum. | Kept in the frozen enum (E.9 is extracted unchanged) and **never emitted**: no path can produce them. |
