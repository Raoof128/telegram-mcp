...
ChatGPT reaches the remote ingress only; it never gets a lease seed. The remote listener must be
reachable over HTTPS at the configured issuer (the owner's tunnel), and the ChatGPT connector
must be registered as a client:

```bash
comms client add --name chatgpt --helper-path ~/.config/comms/chatgpt.seed
```

In ChatGPT, add a custom connector whose MCP URL is `<issuer>/mcp`. When ChatGPT opens the
authorization page, issue the one-time owner code (valid five minutes) and enter it there:

```bash
comms oauth approve
```

A refused or expired code is the correct result of a slow approval; issue a new one.

## Record

Fill every field; a field left `PENDING OWNER` means the run did not happen. Paste this table,
filled, into `docs/verification/comms-v0.3-acceptance/<date>-chatgpt.md` (D39).
