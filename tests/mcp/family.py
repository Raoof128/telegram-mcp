...
_CATALOG = {spec.name: spec for spec in TOOL_CATALOG}


def _served(spec):
    """The spec as the catalog serves it (with next_actions on its output)."""
    return _CATALOG[spec.name]
...
def dispatch_reaches_its_service(spec, example, result):
    ...
    for other in TOOL_CATALOG:
        services.register(
            other.service,
            lambda client, arguments, name=other.service: seen.append((name, arguments)) or result,
        )
    ...
    assert isinstance(structured.pop("next_actions"), list)
    assert seen == [(spec.service, arguments)] and structured == result
